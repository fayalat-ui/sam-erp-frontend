...
class SharePointClient {
  ...
  async getListItems(
    ...
    try {
      ...
      if (filter && filter.trim().length > 0) query = query.filter(filter);
      if (orderBy && orderBy.trim().length > 0) query = query.orderby(orderBy);
      if (top && Number.isFinite(top)) query = query.top(top as number);
